...
inklusive **Lagerort** im Keller. Alles bleibt **lokal auf deinem iPhone**.
...
- 🔎 **Automatische Erkennung auf dem Gerät** (kostenlos, offline) mit Apple Vision:
...
- 💾 Alles wird **lokal auf dem iPhone** gespeichert (SwiftData-Datenbank +
  Etiketten-Fotos), keine Cloud, kein Konto nötig
...
7. Auf der Startseite siehst du alle Flaschen samt **Gesamtzahl**; tippe auf
   die **Lupe** zum Suchen und auf einen Eintrag, um ihn zu **bearbeiten** oder
   zu **löschen**
...
├── Views/
│   ├── BottleListView.swift      # Übersicht aller Flaschen (Suche + Gesamtzahl)
...
│   ├── LabelRecognizer.swift     # Offline-Erkennung: Text + EAN-Barcode (Vision)
│   └── WineAIService.swift       # Optionale KI-Erkennung (Anthropic)
└── Assets.xcassets/              # App-Icon & Akzentfarbe
```

**Technik:** SwiftUI · SwiftData (lokale Speicherung) · Apple Vision
(Text- & Barcode-Erkennung, offline) · Anthropic-API (optionale KI).
Mindest-iOS: 17.0.
